...
#
# Created:     2003/11/03
# RCS-ID:      $Id: wxPyPlot.py,v 1.37 2003-06-27 08:30:47-04 gordon_williams Exp gordon_williams $
# Copyright:   (c) 2002
...
try:
    ...
    print """This module requires the Numeric module, which could not be imported.
...
information on downloading source or binaries."""
    raise ImportError, "Numeric not found"
...
class PolyLine(PolyPoints):
    ...
    def draw(self, dc, printerScale, coord= None):
        ...
        dc.SetPen(wx.wxPen(wx.wxNamedColour(colour), width, style))
        if coord == None:
...
class PolyMarker(PolyPoints):
    ...
    def draw(self, dc, printerScale, coord= None):
        ...
        dc.SetPen(wx.wxPen(wx.wxNamedColour(colour),width))
        if fillcolour:
...
class PlotCanvas(wx.wxWindow):
    ...
    def SaveFile(self, fileName= ''):
        ...
        if string.lower(fileName[-3:]) not in ['bmp','xbm','xpm','png','jpg']:
            ...
            #File name has required exension
    ...
    def GetXY(self,event):
        ...
        screenPos= numpy.array( event.GetPosition())
    ...
    def _getFont(self,size):
        ...
        of = self.GetFont()
        font =  wx.wxFont(s, of.GetFamily(), of.GetStyle(), of.GetWeight())                     
        #font.SetPointSize(s)
        return font
    ...
    def _drawAxes(self, dc, p1, p2, scale, shift, xticks, yticks):
        ...
        dc.SetPen(wx.wxPen(wx.wxNamedColour('BLACK'),penWidth))
...
if __name__ == '__main__':
    ...
    class AppFrame(wx.wxFrame):
        ...
        def OnPlotDraw4(self, event):
            self.resetDefaults()
            self.client.Draw(_draw4Objects())
##            start = time.clock()            #profile
##            for x in range(10):
##                self.client.Draw(_draw4Objects())
##            print "10 plots of Draw4 took: %f sec."%(time.clock() - start) #profile
