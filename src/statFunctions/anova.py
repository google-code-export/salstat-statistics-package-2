...
class oneway(_genericFunc):
    ...
    def _showGui_GetValues(self):
        ...
        if len( self.colNameSelect ) < self.minRequiredCols:
            self.Logg.write(self._(u"you have to select at least %i columns")%self.requiredcols)
            return
    ...
    def _report(self, result):
        ...
        self.outputGrid.addRowData( [self._(u'Selected columns'),], currRow = 1)
        self.outputGrid.addRowData( self.colNameSelect, currRow = 2)
        self.outputGrid.addRowData( [self._(u'Asumptions')],        currRow = 3 )
        self.outputGrid.addRowData( [self._(u'1-The samples are independent.')], currRow = 4 )
        self.outputGrid.addRowData( [self._(u'2-Each sample is from a normally distributed population.')], currRow = 5)
        self.outputGrid.addRowData( [self._(u''''3-The population standard deviations of the groups are all equal.
                                     This property is known as homoscedasticity.''')], currRow= 6)
        self.outputGrid.addRowData( [self._(u'Results')],        currRow = 7)
